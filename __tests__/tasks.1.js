...
describe("Transparent Login", () => {
    ...
    it("Body tag Should contain a background image that covers the whole page", async () => {
        try {
            ...
            console.log('bg size', backgroundSizeProperty)
    ...
    it("A gradient Should be applied to body tag for dark filter effect", async () => {
        try {
            ...
            expect(backgroundProperty[7]).toMatch(/linear-gradient\(.*\)/);
        } catch (err) {
            throw err;
        }
    });
    ...
    it("Font awesome CDN should be present in the Html's head tag", async () => {
        try {
            const fontAwesome = await page.$$eval('link', el => Array.from(el).map(e => e.href));
            expect(fontAwesome[0]).toContain('https://cdnjs.cloudflare.com/ajax/libs/font-awesome');
        } catch (err) {
            throw err;
        }
    });
